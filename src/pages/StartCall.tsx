...
/**
 * Mirror of StartConsult, but for the video-call flow. Once the doctor
 * picks (or creates) a patient, we book a tele appointment for "right
 * now" and jump straight into /call/:appointmentId.
 */
export function StartCall() {
  ...
  const startCallWith = async (patient: Patient) => {
    if (!doctor) return;
    ...
    try {
      ...
      const appt = await createAppointment.mutateAsync({
        ...
        note: 'Started from the Start-call flow',
      });
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Pick an existing patient or register a new one. We'll book a virtual appointment and drop you straight into the call."
      />
      ...
      <Card className="p-4 flex flex-col gap-4">
        ...
        {error && (
          <div className="text-[12px] text-danger bg-danger-soft border border-danger/30 rounded-md px-3 py-2">
            {error}
          </div>
        )}
      </Card>
          ...
          {!isLoading &&
            filtered.map((p) => (
              ...
              >
                ...
                <Video className="h-4 w-4 text-ink-3 shrink-0" />
